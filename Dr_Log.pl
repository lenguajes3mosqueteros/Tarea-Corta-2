...
consulta:- write('Bienvenido a Dr. Log'), nl,
			 ...
			 write('La consulta cuesta $500, se acepta Visa, Mastercard y AmericanExpress'),
...
pattern([_,cual, es, el, tratamiento, de, la,A|_], ['Para tratar la',A, se, debe,':'|T]):-trpattern([_,como, se, trata, el, A|_], ['Podemos tratar el',A, con,':'|T]):-tratar(A,T).
...
pattern([buenas, noches], ['Buenas Noches', 'soy', 'Dr. Log.','Como puedo ayudarle?']).
